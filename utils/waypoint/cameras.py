...
import argparse
import numpy as np
from .follow_waypoint_control import follow_track
...
if __name__ == "__main__":
    ...
    draw_thread = DrawInCarlaThread(world, REFRESH_INTERVAL, PIC_SIZE, CAMERA_SCALING_PARAM, CAMERA_INIT_HEIGHT, DEFAULT_POINT_HEIGHT, use_z, floor_height=FLOOR_HEIGHT)
    draw_thread.start()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_c:
                    ...
                    follow_track(forward = True, create_ego_car = False, reference_points = reference_points, final_track_save_path = args.save_path)

                elif event.key == pygame.K_q:

                    # 倒车启动追踪
                    reference_points = draw_thread.get_reference_points()
                    follow_track(forward = False, create_ego_car = False, reference_points = reference_points, final_track_save_path = args.save_path)
