...
def save_track(new_path, save_path):
...
def follow_track(data_path = "tf.npy", forward = True, create_ego_car = True, reference_points = [], map_name = '', final_track_save_path = ''):
    dT = 0.02
    ...
    if not len(reference_points) == 0:
        ...
        # 无轨迹输入，且文件存在，采用文件数据
        transform, time_stamp = read_data(data_path)

        # 使用插值法修正轨迹
        x, y, z, pitch, yaw, roll = interpolate(transform, time_stamp, dT)
        new_len = x.shape[0]
    ...
    if not len(final_track_save_path) == 0:
        save_track(new_path, final_track_save_path)
